...
public class NationalTeamTest {
	...
	public static void main(String[] args) {
		...
		FootballPlayer foreignPlayer = new FootballPlayer("Foreigner", "ForeignLand", 1, 1, PlayingPosition.DF);
		...
		NationalTeam bigTeam = new NationalTeam();
		for (int i = 0; i < 2 * Constants.MAX_NO_PLAYERS_NATIONAL; i++) {
			FootballPlayer sequentialPlayer = new FootballPlayer("NAME" + i, Constants.DEFAULT_COUNTRY, 10, 100, 
					PlayingPosition.GK);
			bigTeam.registerPlayer(sequentialPlayer);
		}
		if(bigTeam.getRegisteredPlayers().size() > Constants.MAX_NO_PLAYERS_NATIONAL) {
			System.err.println("TEST FAILED: Too many players registered!!");
		}
